<template>
  <ContentWrap>
    ...
    <el-form class="-mb-15px" :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
      ...
      <el-form-item label="币种代码" prop="code">
        <el-input v-model="queryParams.code" placeholder="请输入币种代码" clearable @keyup.enter="handleQuery"
          class="!w-240px" />
      </el-form-item>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      <el-table-column label="币种编号" align="center" prop="id" />
      ...
      <el-table-column label="小数位数" align="center" prop="decimalPlaces" />
      <el-table-column label="状态" align="center" prop="status">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.COMMON_STATUS" :value="scope.row.status" />
        </template>
      </el-table-column>
      <el-table-column label="基础币种" align="center" prop="isBase">
        <template #default="scope">
...
/** ERP 币种列表 */
defineOptions({ name: 'ErpCurrency' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<CurrencyVO[]>([]) // 列表的数据
...
const handleExport = async () => {
  try {
    ...
    const data = await CurrencyApi.exportCurrency(queryParams)
    download.excel(data, '多币种.xls')
  } catch {
  } finally {
